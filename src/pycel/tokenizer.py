...
def shunting_yard(expression):
    ...
    logger.debug("tokens: ", "|".join([x.tvalue for x in tokens]))
    ...
    def po():
        logger.debug("output: ", "|".join([x.tvalue for x in output]))
    def so():
        logger.debug("stack:", "|".join([x.tvalue for x in stack]))
