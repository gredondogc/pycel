...
class Cell(object):
    ...
    def compile(self):
        ...
        if isinstance(self.value, string_types) and not self.formula and not self.python_expression.startswith('"'):
            self.python_expression = '"{}"'.format(self.python_expression)
...
def col2num(col):
    """Convert alpha notation for Excel columns to numbers, e.g., BA -> 53"""
...
def num2col(num):
    """Convert numeric notation for Excel columns to alpha, e.g., 54 -> BB"""
    if num < 1:
...
def almost_equal(v1, v2, tol=0.01):
